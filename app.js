...
const Work = {
  //An object representing the workplace with its members.
  pay: 0, //Accumulated salary.
  doTheWork: function () {
    //Method for performing work and generating pay.
    this.pay += 100;
    updateElements();
  },
  putPayInBank: function () {
    //Puts money from pay into the bank.
    if (Bank.loan > 0) {
      //If worker is in debt 10% goes to clearing the debt, 90% goes to balance.
      Bank.payOffLoanFromOutside(0.1 * this.pay);
      Bank.balance += 0.9 * this.pay;
      this.pay = 0;
    } else {
      //If there is no debt all the cash goes to balance.
  ...
  payOffLoanWithPay: function () {
    //This method uses all the money in pay towards clearing debt, any remaining cash goes to balance.
...
function buy() {
  //A function that allows the user to buy a laptop of their choice! (whichever laptop is selected at the time.)
  const price = parseInt(priceElement.innerText);
  if (price <= Bank.balance) {
    //...assuming they can afford it.
    Bank.balance -= price;
    alert(
      "You are now the proud owner of a brand new " + titleElement.innerText
    );
  } else {
